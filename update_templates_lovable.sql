-- ETAPA 1: Ajustar estrutura da tabela document_templates
ALTER TABLE IF EXISTS public.document_templates ADD COLUMN IF NOT EXISTS section_id text;
ALTER TABLE IF EXISTS public.document_templates ADD COLUMN IF NOT EXISTS agent text;
ALTER TABLE IF EXISTS public.document_templates ADD COLUMN IF NOT EXISTS skill text;
ALTER TABLE IF EXISTS public.document_templates ADD COLUMN IF NOT EXISTS order_index integer;
ALTER TABLE IF EXISTS public.document_templates ADD COLUMN IF NOT EXISTS required boolean DEFAULT false;
ALTER TABLE IF EXISTS public.document_templates ADD COLUMN IF NOT EXISTS instructions text;

-- Remover a restrição UNIQUE antiga de doc_type (já que agora temos várias linhas por tipo)
ALTER TABLE IF EXISTS public.document_templates DROP CONSTRAINT IF EXISTS document_templates_doc_type_key;

-- Adicionar a nova restrição UNIQUE composta por doc_type e section_id
DO $$
BEGIN
    IF NOT EXISTS (
