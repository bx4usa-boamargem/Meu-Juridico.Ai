...
import type {
  ...
  ProcessContext, SkillInput, SkillOutput
} from '../types/index.ts';
...
export async function orchestrateDocument(
  ...
  try {
    ...
    await logAudit(supabase, {
      ...
      new_value: { stage: 'PLAN', doc_type: doc.doc_type },
    });
    ...
    for (const sectionPlan of sectionsToProcess) {
      ...
      const genResult = await stageGenerateSection(
        supabase, doc, sectionPlan, processCtx, sectionMemories, userId
      );
      ...
      if (genResult.success) {
        ...
        const valResult = await stageValidateSection(
          supabase, doc, sectionPlan, genResult.memory!, processCtx, userId
        );
    ...
    const consolidateResult = await stageConsolidate(supabase, doc, sectionMemories, userId);
...
// ─────────────────────────────────────────────
// GENERATE_SECTION: Gera texto de uma seção
// ─────────────────────────────────────────────
...
async function stageGenerateSection(
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      const agent = getAgent(sectionPlan.agent);
      const output = await callClaude(agent.system_prompt, buildGeneratePrompt(skillInput));
...
// ─────────────────────────────────────────────
// VALIDATE_SECTION: Validação jurídica da seção
// ─────────────────────────────────────────────
...
async function stageValidateSection(
  ...
  try {
    const agent = getAgent('AGENT_JURIDICO');
    const output = await callClaude(
      agent.system_prompt,
      buildValidatePrompt(section.rendered_content, sectionMemory.normative_refs, doc.doc_type as string, ragContext)
    );
...
// ─────────────────────────────────────────────
// CONSOLIDATE: Coerência cross-section (Agent 5 / AGENT_CONSOLIDATOR)
// ─────────────────────────────────────────────
...
async function stageConsolidate(
  ...
  try {
    const agent = getAgent('AGENT_CONSOLIDATOR');
    const output = await callClaude(
      agent.system_prompt,
      buildConsolidatePrompt(sectionMemories)
    );

    // Marcar seções com warnings de inconsistência
    if (output.sections_with_warnings?.length > 0) {
      for (const sectionId of output.sections_with_warnings) {
        await supabase.from('document_sections')
    ...
    await logAudit(supabase, {
      ...
      document_id: doc.id as string,
      new_value: { inconsistencies_found: output.inconsistencies?.length || 0 },
    });
...
async function callClaude(systemPrompt: string, userPrompt: string): Promise<Record<string, unknown>> {
  ...
  parsed.cost_usd = (
    (data.usage?.input_tokens || 0) * 0.000003 +
    (data.usage?.output_tokens || 0) * 0.000015
  );
...
function buildGeneratePrompt(input: SkillInput): string {
...
function buildValidatePrompt(
  ...
  normativeRefs: SectionMemory['normative_refs'],
  docType: string,
...
${JSON.stringify(normativeRefs, null, 2)}
...
function buildConsolidatePrompt(memories: SectionMemory[]): string {
  return `Verifique coerência entre todas as seções geradas.
  ...
  "inconsistencies": [
    {
      ...
      "value_a": 150000,
      "value_b": 180000,
      "description": "Valor divergente entre seções",
      "severity": "error"
    }
  ],
  "standardized_terms": {},
