-- ============================================================
-- SPRINT 4 — Painel de Impacto e ROI
-- Tabela roi_benchmarks com benchmarks reais do setor público
...
CREATE POLICY "authenticated_read_benchmarks" ON roi_benchmarks FOR SELECT TO authenticated USING (true);
CREATE POLICY "service_manage_benchmarks" ON roi_benchmarks FOR ALL TO service_role USING (true);

INSERT INTO roi_benchmarks (indicador, valor, unidade, fonte) VALUES
  ...
  ('tempo_mapa_risco_manual_horas',     8.0,   'horas',           'TCU 2022'),
  ...
  ('tempo_mapa_risco_ia_minutos',      15.0,   'minutos',         'MeuJurídico.ai'),
  ...
  ('custo_medio_retrabalho_brl',     2500.0,   'R$',              'estimado TCU')
ON CONFLICT (indicador) DO UPDATE SET valor = EXCLUDED.valor, fonte = EXCLUDED.fonte;
